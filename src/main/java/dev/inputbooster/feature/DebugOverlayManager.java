...
public class DebugOverlayManager {
    ...
    public static void render(GuiGraphicsExtractor ctx) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            Line l = lines.get(i);
            ctx.text(font, l.text, scaledTextX, scaledTextY + i * scaledLineH, l.color);
        }
    ...
    private static void drawKeystrokesGrid(GuiGraphicsExtractor ctx, Font font, int x, int y, Minecraft mc) {
    ...
    private static void drawKey(GuiGraphicsExtractor ctx, Font font, int x, int y, int w, int h, String text, boolean pressed) {
        ...
        ctx.centeredText(font, text, x + w / 2, y + (h - 8) / 2, textColor);
    }
