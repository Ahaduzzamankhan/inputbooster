...
public class InputBoosterScreen extends Screen {
    ...
    private void initPollRateTab(int cx, int top, int bw, int bh, int gap) {
        ...
        addDrawableChild(toggleButton(cx, top + gap * 2, "Burst Mode", InputBoosterConfig.isBurstModeEnabled(), btn -> {
        ...
        addDrawableChild(toggleButton(cx, top + gap * 3, "Combo Keys (Ctrl+1-5)", InputBoosterConfig.isComboKeysEnabled(), btn -> {
    ...
    private void initStatsTab(int cx, int top, int bw, int bh, int gap) {
        addDrawableChild(ButtonWidget.builder(Text.literal("§cReset Peak Latency"), btn ->
    ...
    private void initProfilesTab(int cx, int top, int bw, int bh, int gap) {
        ...
        for (int i = 0; i < profiles.size(); i++) {
            ...
            final int idx = i;
            int rowY = top + gap * i;
            addDrawableChild(ButtonWidget.builder(Text.literal("§a▶ " + p.name()), btn ->
                pm.loadProfile(p.name(), this.client)
        ...
        if (profiles.size() < ProfileManager.MAX_PROFILES) {
            ...
            for (int i = 0; i < quickNames.length; i++) {
                ...
                addDrawableChild(ButtonWidget.builder(Text.literal(qn), btn -> {
                    pm.saveProfile(qn);
                    clearAndInit();
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        if (hasChanges) {
            ctx.drawCenteredTextWithShadow(this.textRenderer,
                Text.literal("§e⚠ Unsaved changes"), this.width / 2, this.height - 56, 0xFFFF55);
        }
    ...
    private void renderStatsContent(DrawContext ctx) {
        ...
        for (String line : lines) {
            ctx.drawTextWithShadow(this.textRenderer, line, cx - 110, y, 0xFFFFFF);
            y += lh;
        }
        renderSparkline(ctx, cx - 110, y, 220, 20, ss.getCpsHistory());
    }

    private void renderSparkline(DrawContext ctx, int x, int y, int w, int h, int[] data) {
        ...
        ctx.drawHorizontalLine(x, x + w - 1, y, 0xFF888888);
        ctx.drawHorizontalLine(x, x + w - 1, y + h - 1, 0xFF888888);
        ctx.drawVerticalLine(x, y, y + h - 1, 0xFF888888);
        ctx.drawVerticalLine(x + w - 1, y, y + h - 1, 0xFF888888);
    }
    ...
    private void updateSliderActive() {
        if (pollSlider != null) {
            ...
            if (presetButtons != null) for (ButtonWidget pb : presetButtons) pb.active = manual;
        }
    }
    ...
    private static class PollRateSlider extends SliderWidget {
        ...
        PollRateSlider(int x, int y, int w, int h, int currentHz) {
            super(x, y, w, h, Text.literal("Poll Rate: " + currentHz + " Hz"), (currentHz - 60) / 940.0);
            this.hz = currentHz;
        }
        ...
        @Override protected void updateMessage() { setMessage(Text.literal("Poll Rate: §e" + hz + " Hz")); }
    ...
    private static class MaxCpsSlider extends SliderWidget {
        private int cps;
        MaxCpsSlider(int x, int y, int w, int h, int currentCps) {
            super(x, y, w, h, Text.literal("Max CPS: " + currentCps), (currentCps - 1) / 19.0);
            this.cps = currentCps;
        }
        @Override protected void updateMessage() { setMessage(Text.literal("Max CPS: §e" + cps)); }
        @Override protected void applyValue() {
            cps = 1 + (int)(value * 19);
    ...
    private static class FpsCheckSlider extends SliderWidget {
        ...
        FpsCheckSlider(int x, int y, int w, int h, int currentTicks) {
            super(x, y, w, h, Text.literal("FPS Check: " + currentTicks + " ticks"), (currentTicks - 1) / 99.0);
            this.ticks = currentTicks;
        }
        @Override protected void updateMessage() { setMessage(Text.literal("FPS Check: §e" + ticks + " ticks")); }
        @Override protected void applyValue() {
